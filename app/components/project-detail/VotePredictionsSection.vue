...
const carouselPredictions = computed(() => {
...
function toPrediction(item: unknown): VotePrediction {
  return item as VotePrediction
}
</script>
  ...
  >
    ...
    <template v-else>
      ...
      <UCarousel
        ...
        arrows
        dots
        :items="carouselPredictions"
      >
        <div v-for="prediction in [toPrediction(item)]" :key="prediction.id">
          <UPageCard
            variant="outline"
            class="h-full border-primary/20 cursor-pointer transition-all m-1"
