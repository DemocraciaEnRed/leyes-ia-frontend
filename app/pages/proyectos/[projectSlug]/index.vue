...
const { data: featuredSurveyResponse } = await useFetch<{ hasFeaturedSurvey: boolean, featuredSurvey: PublicSurvey | null }>(
	() => `/api/backend/hub/projects/slug/${projectSlug.value}/featured-survey`,
	{
		watch: [projectSlug]
	}
)

const { data: availableSurveysResponse } = await useFetch<{ surveys: PublicSurvey[] }>(
	() => `/api/backend/hub/projects/slug/${projectSlug.value}/available-surveys`,
	{
		watch: [projectSlug]
	}
)
...
const featuredSurvey = computed(() => featuredSurveyResponse.value?.featuredSurvey || null)
const availableSurveys = computed(() => availableSurveysResponse.value?.surveys || [])
</script>
	...
	<NuxtLayout name="default">
		...
		<UContainer class="max-w-5xl py-10 space-y-8">
			...
			<template v-else>
				...
				<USeparator icon="lucide:chart-no-axes-column" class="my-2" v-if="availableSurveys.length > 0 || featuredSurvey" />
