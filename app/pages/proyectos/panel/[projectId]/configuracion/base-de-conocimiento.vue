...
const searchVectorDatabase = async () => {
  try {
    ...
    $fetch(`/api/backend/projects/${projectId}/manage/knowledge-base/retrieve`, {
      method: 'POST',
      body: {
...
<template>
    ...
    />
    <UPageBody>
      <ProjectKnowledgeBaseDashboardStats
      ...
      <p>
        Recupera documentos relevantes de la base de conocimiento mediante búsqueda semántica: busca por
        significado (no solo por palabras exactas) para encontrar información relacionada.
      </p>
      ...
      >
        ...
        <UFormField label="Palabras clave para búsqueda en base de conocimiento">
          <UInput
            v-model="searchQuery"
            placeholder="Ejemplo: educación, salud, medio ambiente"
            class="w-full"
          />
        </UFormField>
        <!-- NumResults: Cantidad de resultados -->
        <UFormField
          label="Cantidad de resultados a mostrar"
          class="w-full"
        >
          <UInputNumber
            v-model="numResults"
        ...
        <UFormField
          label="Peso de la similitud (Alpha)"
        ...
        >
          ...
          <template #help>
            <p class="mt-2">
              Alpha controla cuánto pesa la búsqueda por significado (vectorial) frente a la búsqueda por palabras:
            </p>
            ...
            <p class="mt-2">
              <strong>Ejemplos:</strong> <em>0.25</em> (prioriza palabras), <em>0.5</em> (equilibrado), <em>0.75</em> (prioriza significado).
            </p>
            <p class="mt-1 text-sm">
              Sugerencia: <strong>0.6–0.9</strong> para resultados mása "inteligentes"; <strong>0.2-0.4</strong> si quieres resultados más literales.
            </p>
          </template>
        </UFormField>
        <UButton
      ...
      >
        <UPageCard
          ...
          :key="`${item?.metadata?.item_name || 'item'}-${index}`"
          variant="outline"
          class="bg-elevated"
        >
          <div class="space-y-3">
            ...
            <div class="flex flex-wrap gap-2">
              <UBadge
                v-if="item?.metadata?.chunk_category"
                color="primary"
                variant="subtle"
              >
              ...
              <UBadge
                v-if="item?.metadata?.page_number"
                color="neutral"
                variant="subtle"
              >
              ...
              <UBadge
                v-if="item?.metadata?.ingested_timestamp"
                color="neutral"
                variant="subtle"
              >
