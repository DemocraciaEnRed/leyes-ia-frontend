...
<template>
    <UDashboardPanel>
        <template #body>
            <UContainer>
                <h1 class="text-4xl font-bold mb-4 text-center">Hola, ¿cómo puedo ayudarte?</h1>
                <div class="flex justify-center mb-4">
                    <UButton v-if="!sessionId" color="primary" @click="newSession" label="Iniciar nueva sesión de chat" />
                </div>
                ...
                <UChatMessages 
                    ...
                    >
                    <template #indicator>
                        <UButton class="px-0" color="neutral" variant="link" loading loading-icon="i-lucide-loader"
                        label="Thinking..." />
                    </template>
        ...
        <template #footer v-if="sessionId">
            <UContainer class="pb-4 sm:pb-6">
                <UChatPrompt variant="subtle" v-model="input" @submit="submitMessage">
                    <UChatPromptSubmit :status="statusPrompt" @stop="stopGenerating" @reload="submitMessage" />
                </UChatPrompt>
            </UContainer>
        </template>
    </UDashboardPanel>
