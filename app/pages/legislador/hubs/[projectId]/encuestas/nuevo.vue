<script setup>
    ...
    const generateContentWithAI = async () => {
        ...
        try {
            ...
            const response = await $fetch(`/api/backend/projects/${projectId}/surveys/generate`, {
                method: 'POST',
                body: {
                    projectId: projectId,
    ...
    const regenerateContentWithAI = () => {
        ...
        generatingContent.value = true;
        $fetch(`/api/backend/projects/${projectId}/surveys/regenerate`, {
            method: 'POST',
            body: {
    ...
    const saveSurvey = async () => {
        ...
        try {
            const response = await $authFetch(`/api/backend/projects/${projectId}/surveys`, {
                method: 'POST',
                body: {
...
<template>
    <NuxtLayout name="panel-proyecto">
        ...
        />
        <UPageBody>
            <UPageCard variant="subtle" class="" title="1. Datos basicos" description="Completá los campos a continuación para definir los datos básicos de la encuesta. El título es importante para identificar la encuesta, pero si lo dejás vacío la IA generará un título por vos.">
                ...
                <UFormField label="Descripción de la encuesta">
                    <UTextarea autoresize v-model="surveyAbout" placeholder="Escribe una breve descripción de la encuesta aquí" class="w-full" />
                </UFormField>
            ...
            <UPageCard variant="subtle" title="2. Configurar mensaje de bienvenida" description="Personalizá el mensaje de bienvenida que verán los encuestados al iniciar la encuesta. Este mensaje es clave para motivar la participación y explicar brevemente el propósito de la encuesta. Si lo dejás vacío, se utilizará un mensaje de bienvenida genérico.">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    <div class="flex flex-col gap-4">
                        <UFormField label="Mensaje de bienvenida para los encuestados">
                            <UInput v-model="surveyWelcomeTitle" placeholder="Escribe el mensaje de bienvenida para los encuestados aquí" class="w-full" />
                        </UFormField>
                        <UFormField label="Subtítulo o instrucciones iniciales para los encuestados">
                            <UTextarea autoresize v-model="surveyWelcomeSubtitle" placeholder="Escribe el subtítulo o instrucciones iniciales para los encuestados aquí" class="w-full" />
                        </UFormField>
                    </div>
                    <UPageCard variant="outline" class="">
                        <div class="flex flex-col items-center justify-center text-center space-y-2 min-h-75">
                            ...
                            <p class="text-md text-muted whitespace-break-spaces">{{ surveyWelcomeSubtitle }}</p>
                            <br/>
                            <UButton color="primary" variant="solid" trailing-icon="lucide:arrow-right">
                                Comenzar
                            </UButton>
                        </div>
                    </UPageCard>
            ...
            <UPageCard variant="subtle" title="3. Generar preguntas con IA" description="Completá los campos para que la IA genere preguntas alineadas a tu objetivo. Luego podés editar las preguntas generadas o agregar nuevas preguntas manualmente.">
                <UFormField label="Objetivo de la encuesta">
                    ...
                    <UTextarea autoresize v-model="surveyObjective" class="w-full" :disabled="generatingContent || savingSurvey || alreadyContentGenerated"
                        placeholder="Describa el objetivo aquí" :autoresize="true" />
                </UFormField>
                <UFormField label="Audiencia objetivo">
                    ...
                    <UTextarea autoresize v-model="surveyTargetAudience" class="w-full" :disabled="generatingContent || savingSurvey || alreadyContentGenerated"
                ...
                <UFormField label="Contexto adicional">
                    ...
                    <UTextarea autoresize v-model="surveyContext" class="w-full" :disabled="generatingContent || savingSurvey || alreadyContentGenerated"
                        placeholder="Proporcione contexto adicional aquí" />
                </UFormField>
                <UFormField label="Cantidad de preguntas" help="Elija cuántas preguntas debe tener la encuesta (mínimo 4, máximo 12)">
                    <UInputNumber v-model="surveyQuestionCount" :min="5" :max="12" :step="1" :disabled="generatingContent || savingSurvey || alreadyContentGenerated" class="w-full" />
                </UFormField>
                <UFormField label="Instrucciones para la IA">
                    ...
                    <UTextarea autoresize v-model="surveyUserInstructions" class="w-full" :disabled="generatingContent || savingSurvey || alreadyContentGenerated"
                ...
                <UFormField :ui="{labelWrapper: 'w-full m-0'}">
                    ...
                    <div class="flex flex-col gap-2">
                        <div v-for="(q, idx) in surveyRequiredQuestions" :key="idx" class="flex gap-2">
                            <UInput v-model="surveyRequiredQuestions[idx]" class="flex-1"
                            placeholder="Escriba aquí una pregunta requerida" :disabled="generatingContent || savingSurvey || alreadyContentGenerated" />
                            <UButton variant="outline" color="neutral" @click="removeRequiredQuestion(idx)"
                            :disabled="generatingContent || savingSurvey || alreadyContentGenerated"  icon="lucide:trash-2"></UButton>
                        </div>
                        <UButton color="neutral" variant="subtle" class="justify-center cursor-pointer"  @click="addRequiredQuestion" :disabled="generatingContent || savingSurvey || alreadyContentGenerated" icon="lucide:plus-circle"> Añadir pregunta</UButton>
                    </div>
                </UFormField>
            ...
            <UPageCard variant="outline" title="4. Preguntas generadas" description="A continuación se muestran las preguntas generadas por la IA. Puede editarlas, eliminarlas o reordenarlas antes de guardar la encuesta." v-if="survey && survey.questions">
                <UEmpty
                ...
                <UPageCard variant="outline" class=" sticky bottom-10 mt-5 z-10 mx-4" v-if="alreadyContentGenerated">
