...
const generateContentWithAI = () => {
    ...
    // /api/backend/projects/:projectId/generate-fields
    generatingContent.value = true;
    $fetch(`/api/backend/projects/${projectId}/generate-fields`, {
        method: 'POST'
    }).then(response => {
...
const regenerateContentWithAI = () => {
    ...
    generatingContent.value = true;
    $fetch(`/api/backend/projects/${projectId}/regenerate-fields`, {
        method: 'POST',
        body: {
...
const saveProject = () => {
    ...
    $fetch(`/api/backend/projects/${projectId}/save-fields`, {
        method: 'PUT',
        body: payload
    }).then(response => {
        ...
        navigateTo(`/legislador/hubs/${projectId}`);
    }).catch(error => {
...
<template>
    <NuxtLayout name="panel-proyecto">
        <UPageHeader title="Editar resumen" class="mb-6"
        ...
        <UPageBody>
                <UPageCard variant="outline" class="bg-elevated">
                    ...
                    <UChatPrompt v-model="userPrompt" @submit="regenerateContentWithAI" v-if="alreadyContentGenerated" :loading="generatingContent" placeholder="¿Qué necesita que edite la AI de lo que ya generó?" :disabled="generatingContent || savingProject">
