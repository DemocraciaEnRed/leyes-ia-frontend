...
const { data: dataResponse, status, error, refresh } = await useFetch(`/api/backend/projects/${projectId}`);
...
const links = ref([
    ...
    },
    {
        label: 'Editar',
        icon: 'lucide:edit-3',
        to: `/legislador/hubs/${projectId}/resumen/editar`
    }
])
...
<template>
    <NuxtLayout name="panel-proyecto">
        <UPageHeader title="Resumen del proyecto" 
        ...
        <UPageBody v-if="status == 'success' && dataResponse.project.status == 'ready'" >
            ...
            <MDC :value="dataResponse?.project.description" class="px-5 py-1 rounded-xl bg-muted/50" />
            <h1 class="text-2xl font-bold mb-4">Categoria</h1>
            <UBadge size="xl" variant="subtle" color="neutral">{{ dataResponse?.project.category }}</UBadge>
